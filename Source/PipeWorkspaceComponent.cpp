...
{
    Flow() = default;
    Flow (IVec3 positionIn,
          FVec3 fallPositionIn,
          uint8_t directionIn,
          double progressIn = 0.0,
          bool fallingIn = false,
          bool deadIn = false,
          double segmentBeatsIn = 0.5,
          double delayBeatsIn = 0.0,
          float accentIn = 1.0f,
          uint8_t echoGenerationIn = 0)
        : position (positionIn),
          fallPosition (fallPositionIn),
          direction (directionIn),
          progress (progressIn),
          falling (fallingIn),
          dead (deadIn),
          segmentBeats (segmentBeatsIn),
          delayBeats (delayBeatsIn),
          accent (accentIn),
          echoGeneration (echoGenerationIn)
    {
    }
    ...
    bool falling = false;
    bool dead = false;
    double segmentBeats = 0.5;
    double delayBeats = 0.0;
    float accent = 1.0f;
    uint8_t echoGeneration = 0;
};
...
public:
    ...
    static inline const juce::Colour bg       { 0xff08110e };
    static inline const juce::Colour panel    { 0xff0d1714 };
    static inline const juce::Colour panel2   { 0xff14201c };
    static inline const juce::Colour line     { 0xff28483f };
    static inline const juce::Colour lineDim  { 0xff172b25 };
...
class PipeWorkspaceComponent final : public juce::AudioAppComponent,
...
public:
    ...
    juce::var createWorkspaceState() { return createPatch(); }
    bool applyWorkspaceState (const juce::var& state) { return loadPatchFromVar (state); }
    void setWorkspaceTempo (double newTempo)
    {
        bpm = juce::jlimit (20.0, 400.0, newTempo);
...
private:
    ...
    {
    public:
        explicit LargeCodeWindow (PipeWorkspaceComponent& ownerToUse)
    ...
    static constexpr size_t maxFlows = 128;
    static constexpr double pipeStepBeats = 0.5;
    static constexpr double junctionEchoDelayBeats = 0.5;
    static constexpr uint8_t maxJunctionEchoGenerations = 2;
    ...
    {
        return R"SC(|out = 0, pitch = 60, amp = 0.18, sustain = 0.35, pan = 0, brightness = 0.2, water = 0.5, height = 0.5, falling = 0, accent = 1, rhythm = 0.5, echo = 0|
    var freq = pitch.midicps;
    var env = EnvGen.kr(Env.perc(0.004, (sustain * (0.75 + rhythm)).max(0.04), curve: -5), doneAction: 2);
    ...
    sig = (sig + shimmer + (SinOsc.ar(freq * 2.01) * falling * 0.18)) * env * amp * accent * (0.75 + height * 0.5);
...
#X connect 5 0 6 1;
    ...
    void applyToolToCell (Cell cell)
    {
        ...
        else if (selectedTool == Tool::valve)
        {
            ...
            setStatus (network.hasValve (voxel) ? "Disc added" : "Disc removed");
            showScProgramForSelection();
        }
    ...
    void setPlaying (bool shouldPlay)
    {
        ...
        setStatus (playing ? "Water running in eighths" : "Stopped");
        updateButtonStates();
        repaint();
    }
    ...
    void updateFlow (double dt)
    {
        ...
        const auto elapsedBeats = dt / beat;
        emitAccumulator += dt;
        ...
        for (auto& flow : flows)
        {
            auto remainingBeats = elapsedBeats;
            if (flow.delayBeats > 0.0)
            {
                const auto consumed = juce::jmin (flow.delayBeats, remainingBeats);
                flow.delayBeats -= consumed;
                remainingBeats -= consumed;
            }

            if (remainingBeats <= 0.0)
                continue;

            if (flow.falling)
            {
                advanceFallingFlow (flow, remainingBeats);
                continue;
            }

            flow.progress += remainingBeats / juce::jmax (0.125, flow.segmentBeats);
        ...
        if (! queuedFlows.empty() && flows.size() < maxFlows)
        {
            const auto available = maxFlows - flows.size();
            const auto count = juce::jmin (available, queuedFlows.size());
            flows.insert (flows.end(), queuedFlows.begin(), queuedFlows.begin() + (std::ptrdiff_t) count);
        }
    }
    ...
    void emitFromTaps()
    {
        for (const auto tap : network.taps)
        {
            ...
            flows.push_back ({ tap, { (float) tap.x, (float) tap.y, (float) tap.z }, choice, 0.0, false, false, pipeStepBeats });
        }
    }
    ...
    void advanceFlow (Flow& flow)
    {
        ...
        {
            const auto choice = exits[(size_t) juce::Random::getSystemRandom().nextInt ((int) exits.size())];
            queueJunctionEchoes (flow, exits, choice);
            flow.direction = choice;
            flow.segmentBeats = pipeStepBeats;
            return;
        }
    ...
    void queueJunctionEchoes (const Flow& source, const std::vector<uint8_t>& exits, uint8_t primaryExit)
    {
        if (exits.size() < 2 || source.echoGeneration >= maxJunctionEchoGenerations)
            return;

        auto echoIndex = 0;
        for (const auto exit : exits)
        {
            if (exit == primaryExit || flows.size() + queuedFlows.size() >= maxFlows)
                continue;
            ...
            echo.falling = false;
            echo.dead = false;
            echo.segmentBeats = pipeStepBeats;
            echo.delayBeats = junctionEchoDelayBeats * (double) (echoIndex + 1);
            echo.accent = juce::jlimit (0.24f, 1.0f, source.accent * 0.68f);
            echo.echoGeneration = (uint8_t) (source.echoGeneration + 1);
            echo.fallPosition = { (float) source.position.x, (float) source.position.y, (float) source.position.z };
            queuedFlows.push_back (echo);
    ...
    void advanceFallingFlow (Flow& flow, double beatFraction)
    {
        ...
        const auto distance = (float) (beatFraction * 4.0);
        const FVec3 next {
    ...
    void triggerValve (IVec3 voxel, const Flow& flow)
    {
        ...
        if (soundMode == SoundMode::superCollider && scEngine.isReady() && valve != nullptr && ensureValveScProgram (*valve))
        {
            ...
            fields.durationTicks = flow.falling ? 2u : (std::uint64_t) juce::jmax (1, juce::roundToInt (flow.segmentBeats * 2.0));
            ...
            fields.parameters["accent"] = juce::String (flow.accent, 3);
            fields.parameters["rhythm"] = juce::String ((float) flow.segmentBeats, 3);
            fields.parameters["echo"] = juce::String ((int) flow.echoGeneration);
    ...
    void drawMarkers2D (juce::Graphics& g, juce::Rectangle<float> grid, float cell)
    {
        ...
        for (const auto& valve : network.valves)
        {
            ...
            {
                const auto centre = cellCentre (grid, cell, *display);
                const auto radius = cell * 0.26f;
                const auto discBounds = juce::Rectangle<float> (radius * 2.0f, radius * 2.0f).withCentre (centre);
                g.setColour (PipeLookAndFeel::aqua.withAlpha (0.18f));
                g.fillEllipse (discBounds.expanded (radius * 0.42f));
                g.setColour (PipeLookAndFeel::sky);
                g.fillEllipse (discBounds);
                g.setColour (PipeLookAndFeel::pink.withAlpha (0.94f));
                g.fillEllipse (discBounds.reduced (radius * 0.26f));
                g.setColour (PipeLookAndFeel::lemon.withAlpha (0.95f));
    ...
    void drawFlows2D (juce::Graphics& g, juce::Rectangle<float> grid, float cell)
    {
        for (const auto& flow : flows)
        {
            if (flow.falling)
            {
                ...
                const auto radius = cell * (0.10f + 0.04f * flow.accent);

                g.setColour (PipeLookAndFeel::water.withAlpha (0.10f + 0.08f * flow.accent));
                g.fillEllipse ({ point.x - radius * 1.8f, point.y - radius * 1.8f, radius * 3.6f, radius * 3.6f });
                g.setColour (PipeLookAndFeel::water.withAlpha (0.48f + 0.52f * flow.accent));
            ...
            const auto radius = cell * (0.11f + 0.06f * flow.accent);

            g.setColour (PipeLookAndFeel::water.withAlpha ((flow.delayBeats > 0.0 ? 0.10f : 0.18f) + 0.08f * flow.accent));
            g.fillEllipse ({ point.x - radius * 2.0f, point.y - radius * 2.0f, radius * 4.0f, radius * 4.0f });
            g.setColour (PipeLookAndFeel::water.withAlpha ((flow.delayBeats > 0.0 ? 0.42f : 0.62f) + 0.38f * flow.accent));
            g.fillEllipse ({ point.x - radius, point.y - radius, radius * 2.0f, radius * 2.0f });
        }
    }
    ...
    void drawPreviewFlows (juce::Graphics& g, juce::Rectangle<float> area)
    {
        for (const auto& flow : flows)
        {
            ...
            const auto radius = 2.2f + 1.0f * flow.accent;
            g.setColour (PipeLookAndFeel::water.withAlpha ((flow.delayBeats > 0.0 ? 0.10f : 0.16f) + 0.08f * flow.accent));
            g.fillEllipse ({ point.x - radius * 2.0f, point.y - radius * 2.0f, radius * 4.0f, radius * 4.0f });
            g.setColour (PipeLookAndFeel::water.brighter (0.22f).withAlpha ((flow.delayBeats > 0.0 ? 0.48f : 0.68f) + 0.32f * flow.accent));
            g.fillEllipse ({ point.x - radius, point.y - radius, radius * 2.0f, radius * 2.0f });
        }
    }
...
{
    return std::make_unique<PipeWorkspaceComponent>();
}
...
juce::var createPipeWorkspaceState (juce::Component& component)
{
    if (auto* workspace = dynamic_cast<PipeWorkspaceComponent*> (&component))
        return workspace->createWorkspaceState();
    return {};
}
...
{
    if (auto* workspace = dynamic_cast<PipeWorkspaceComponent*> (&component))
        return workspace->applyWorkspaceState (state);
    return false;
}
...
{
    if (auto* workspace = dynamic_cast<PipeWorkspaceComponent*> (&component))
        workspace->setWorkspaceRunning (running);
}
